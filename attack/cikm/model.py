...
from attack.surrogate.gcn import GCN_without_norm
...
class RelationGCNs(nn.Module):
    def __init__(self, category, example_graph: dgl.DGLHeteroGraph, h_dict, in_size, out_size, device):
        ...
        for item in example_graph.canonical_etypes:
            ...
            adj_dict[source][target] = example_graph.adj(etype=item[1]).to_dense().to_sparse()

        self.input_layer_shape = dict([(k, self.in_size) for k in self.h_dict.keys()])
        self.hidden_layer_shape = [dict.fromkeys(self.h_dict.keys(), l_hid) for l_hid in [16, 8]]
    ...
    def forward(self, etype_adj_dict):
        feature = self.h_dict
        logits, embedding_dict, attention_dict = self.SHGP.forward(feature, etype_adj_dict)
...
class GCNFreeCompuse(torch.nn.Module):
    def __init__(self, category, example_graph: dgl.DGLHeteroGraph, h_dict, out_size, device):
        super().__init__()
        self.example_graph = example_graph
        self.device = device
        self.weight_dict = {}
        ...
        for item in example_graph.canonical_etypes:
            ...
            self.weight_dict[etype] = torch.nn.Parameter(
                torch.randn(example_graph.num_nodes(source_ntype), example_graph.num_nodes(target_ntype)).to(device))
            self.adj_pos[etype] = [range_of_src, range_of_tar]

        self.zero_adj_example = torch.zeros(example_graph.num_nodes(), example_graph.num_nodes(), device=device)
        self.feature_build = HeteroFeature({category: h_dict[category]}, get_nodes_dict(example_graph),
                                           32).to(self.device)
        self.zero_feat_example = torch.zeros(example_graph.num_nodes(), self.feature_build.embed_size, device=device)
        self.fuse_graph = torch.nn.Linear(len(example_graph.etypes), 1, bias=False)
        self.gcn = GCN_without_norm(self.device, in_size=32, out_size=out_size, hidden_size=16).to(self.device)
    ...
    def forward(self, etype_adj_dict):
        ...
        for k, v in etype_adj_dict.items():
            ...
            # todo
            zero_adj_example[pos_str[0]:pos_str[1], pos_end[0]:pos_end[1]] += self.weight_dict[k] * v
            cat_adj_example[pos_str[0]:pos_str[1], pos_end[0]:pos_end[1]] += v
        for ntype in self.example_graph.ntypes:
            and_end_nid_by_ntype = get_start_and_end_nid_by_ntype(self.example_graph, ntype)
            zero_feat_example[and_end_nid_by_ntype[0]:and_end_nid_by_ntype[1]] = feature[ntype]
...
class HeteMetapahAndMulti(torch.nn.Module):
    def __init__(self, category, example_graph, meta_paths, h_dict, in_size, out_size, hidden_size=16, device='cuda:0'):
        super().__init__()
        self.category = category
        self.example_graph = example_graph
        ...
        self.device = device
        self.GCNInner = GCNInner(meta_paths, in_size, out_size, hidden_size=hidden_size, device=device)
        self.GCNFreeCompuse = GCNFreeCompuse(category=category, example_graph=example_graph, h_dict=h_dict,
                                             device=device, out_size=out_size)
    ...
    def forward(self, etype_adj_dict, X):
        ...
        start_end_nid_by_ntype = get_start_and_end_nid_by_ntype(self.example_graph, self.category)
        gcn_free_compuse_forward_part = gcn_free_compuse_forward[start_end_nid_by_ntype[0]: start_end_nid_by_ntype[1]]

        cated_input = torch.stack([gcn_inner_forward, gcn_free_compuse_forward_part], dim=-1)
...
class JustHeteMetapah(HeteMetapahAndMulti):
    def __init__(self, category, example_graph, meta_paths, h_dict, in_size, out_size, hidden_size=16,
                 device='cuda:0'):
        super().__init__(category, example_graph, meta_paths, h_dict, in_size, out_size, hidden_size, device)
