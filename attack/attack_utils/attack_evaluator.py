...
def eval_transfer(hg: dgl.DGLHeteroGraph, flow: NodeClassification):
    ...
    for model_name in all_models:
        ...
        experiment = Experiment(model=model_name, dataset=dataset_name, task=flow.args.task, gpu=flow.new_args.gpu,
                                use_best_config=True, load_from_pretrained=False)

        new_result, new_flow = experiment.run()
        new_ori_micro_f1, new_ori_macro_f1, _, _ = eval_evasion(example_graph, new_flow, flow.test_idx)
        new_victim_micro_f1, new_victim_macro_f1, _, _ = eval_evasion(victim_graph, new_flow, flow.test_idx)
        ...
        print(f'transfer to model {model_name}, micro_down is {micro_down}, macro_down is {macro_down}')
...
def eval_evasion(hg: dgl.DGLHeteroGraph, flow: NodeClassification, target_nodes: [int] = None,
...
def extract_targets(flow: NodeClassification):
    ...
    other = numpy.random.choice(other, 21, replace=False).tolist()

    return high + low + other


def is_attack_success_on_single_node(flow: NodeClassification, hg: dgl.DGLHeteroGraph, target_node: int):
