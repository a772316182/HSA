...
from attack.attack_utils.hg_info_extract import get_connect_type_matrix, get_filtered_etype, get_etype_dict
...
def normalize_grad_n2v(data: [dict], beta: float):
    ...
    for etype in etypes:
        ...
        n2v_data_splict_by_etypes[etype] = [item['similarity_score'] for item in data_item]
        ...
        z_scored_grad_data_splict_by_etypes[etype] = scipy.stats.zscore(grad_data_splict_by_etypes[etype])
        z_scored_n2v_data_splict_by_etypes[etype] = scipy.stats.zscore(n2v_data_splict_by_etypes[etype])

        index_of_each_etypes[etype] = 0

    for i in range(len(data)):
        data_i_etype_ = data[i]['etype']
        by_etypes_data_i_etype__z_scored_grad_data_splict_by_etypes = z_scored_grad_data_splict_by_etypes[data_i_etype_]
        by_etypes_data_i_etype__z_scored_n2v_data_splict_by_etypes = z_scored_n2v_data_splict_by_etypes[data_i_etype_]

        data[i]['z_scored_grad_value'] = by_etypes_data_i_etype__z_scored_grad_data_splict_by_etypes[
            index_of_each_etypes[data_i_etype_]]
        data[i]['z_scored_similarity_score'] = by_etypes_data_i_etype__z_scored_n2v_data_splict_by_etypes[
            index_of_each_etypes[data_i_etype_]]

        if data[i]['has_edge']:
            # 有边，新增元邻居，此时，新的元邻居要和原来的差距大，即similarity小
            data[i]['mix_value'] = grad_means_by_etypes[data_i_etype_] * (
                    data[i]['z_scored_grad_value'] - beta * grad_stds_by_etypes[data_i_etype_] * data[i][
                'z_scored_similarity_score'])
        else:
...
def remove_all_edges_by_etype(hg: dgl.DGLHeteroGraph, etype: str, etype_reverse: str, device: str):
    hg = hg.clone()
    ...
    etyped_eids_reverse = hg.edge_ids(u=etyped_index[:, 1], v=etyped_index[:, 0], etype=etype_reverse)
...
def from_adj_dict_to_dgl_graph(example_graph: dgl.DGLHeteroGraph, adj_dict: {str, torch.Tensor}):
    etype_dict = get_etype_dict(example_graph)
    num_nodes_dict = {ntype: example_graph.number_of_nodes(ntype) for ntype in example_graph.ntypes}
...
def random_flip_hg_auto_reverse(clean_hg: dgl.DGLHeteroGraph, radio=0.05, etype=None):
    ...
    if etype is None:
        ...
        for etype in filtered_type:
            ...
            sources = torch.randint(low=0, high=hg.num_nodes(source_type_) - 1, size=[num_purb])
            targets = torch.randint(low=0, high=hg.num_nodes(target_type_) - 1, size=[num_purb])
            ...
            etype_modified_adj = torch.where(etype_modified_adj == -1, torch.ones_like(etype_ori_adj),
                                             etype_modified_adj)
...
def flip_on_hg(etype, graph_for_modify: dgl.DGLHeteroGraph, source, target, etype_reverse=None, auto_reverse=False):
    purb_counter = 0
    ...
    if graph_for_modify.has_edges_between(int(source), int(target), etype=etype_canonical_etypes):
        # 有边
        graph_for_modify.remove_edges(
            eids=graph_for_modify.edge_ids(source, target, etype=etype_canonical_etypes),
            etype=etype_canonical_etypes
        )
        purb_counter += 1
        if auto_reverse:
            graph_for_modify.remove_edges(
                eids=graph_for_modify.edge_ids(target, source, etype=etype_reverse_canonical_etypes),
                etype=etype_reverse_canonical_etypes
            )
            purb_counter += 1
    else:
        # 无边
        graph_for_modify.add_edges(
            source, target, etype=etype
        )
        purb_counter += 1
        if auto_reverse:
            graph_for_modify.add_edges(
                target, source, etype=etype_reverse
            )
            purb_counter += 1
