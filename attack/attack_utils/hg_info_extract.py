...
def extract_metapath_from_dataset_for_surrogate(flow: NodeClassification):
    ...
    if dataset_name == 'acm4GTN':
        meta_paths = [['paper-subject', 'subject-paper', 'paper-subject', 'subject-paper']]
    elif dataset_name == 'imdb4GTN':
        # meta_paths = [['movie-director', 'director-movie'],
        #               ['movie-actor', 'actor-movie']]
        meta_paths = [['movie-actor', 'actor-movie', 'movie-director', 'director-movie']]
        ...
        meta_paths = []
        for meta_path in list(extract_metapaths(category, flow.hg.canonical_etypes).values()):
            meta_paths.append([paths[1] for paths in meta_path])
    return feat.to(flow.device), meta_paths


def generate_mata_path_reachable_adjs(flow: NodeClassification, hg: dgl.DGLHeteroGraph, numpy_format: bool = False,
    ...
    for i in range(len(meta_paths)):
        ...
        meta_path_reachable = meta_path_reachable + torch.eye(meta_path_reachable.shape[0]).cpu()
        meta_path_reachable[torch.where(meta_path_reachable > 1)] = 1

        check_target = dgl.metapath_reachable_graph(hg.cpu(), metapath=meta_path).adj().to_dense() + torch.eye(
            meta_path_reachable.shape[0], device='cpu')
        check_target[torch.where(check_target > 1)] = 1
        assert bool(torch.all(
            check_target == meta_path_reachable.cpu()))
        if edge_index_format:
            metapath_reachable_graph_adj_dict[str(meta_path)] = torch.nonzero(meta_path_reachable).long().T
        if numpy_format:
            metapath_reachable_graph_adj_dict[str(meta_path)] = meta_path_reachable.cpu().numpy()
        else:
            metapath_reachable_graph_adj_dict[str(meta_path)] = meta_path_reachable.cpu()
    return metapath_reachable_graph_adj_dict


def get_connect_type_matrix(hg: dgl.DGLHeteroGraph, ntypes):
    connect_type_matrix = pd.DataFrame(
        index=ntypes,
        columns=ntypes,
        dtype=str
    )
...
def get_node_num_dict(hg: dgl.DGLHeteroGraph):
    return dict(zip(
        hg.ntypes, [hg.num_nodes(ntype=ntype) for ntype in hg.ntypes]
    ))


def get_edge_num_dict(hg: dgl.DGLHeteroGraph):
    return dict(zip(
        hg.etypes, [hg.num_edges(etype=etype) for etype in hg.etypes]
    ))
...
def get_etype_adj_dict(hg: dgl.DGLHeteroGraph, device):
    return dict(zip(hg.etypes, [hg.adj(etype).to_dense().detach().clone().to(device) for etype in hg.etypes]))
...
def get_filtered_etype(hg: dgl.DGLHeteroGraph, ntypes: [str]):
    ...
    for item in list(itertools.permutations(ntypes, 2)):
        if [item[0], item[1]] not in filtered_source_target_ntype and [item[1],
                                                                       item[0]] not in filtered_source_target_ntype:
...
def extract_target_nodes(flow: NodeClassification, num_of_target=40):
    ...
    to_cpu_test_idx__numpy = numpy.column_stack(
        [test_idx, pred_label, ground_truth_label[test_idx]]).tolist()
    for i in range(len(to_cpu_test_idx__numpy)):
        if to_cpu_test_idx__numpy[i][1] == to_cpu_test_idx__numpy[i][2]:
            pred_corrected_test_nodes.append(
                to_cpu_test_idx__numpy[i][0]
            )
    # 从中采样40个作为目标节点
