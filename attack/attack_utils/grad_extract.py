...
def get_grad_GTN_dgl(flow, hg, surrogate_model_inner_compose, surrogate_model_outer_compose, mask=None):
    ...
    outer_compose_forward = surrogate_model_outer_compose.forward(graph_list, feat, category_idx)
    ...
    for i, v in enumerate(graph_list):
        if i != len(etypes):
            k = etypes[i]
            etype_grad_dict[k] = torch.autograd.grad(loss, v['adj'], retain_graph=True)[0]
...
def get_logits_HGCN_dgl(flow, hg, surrogate_model_inner_compose, surrogate_model_outer_compose, mask=None):
    surrogate_model_outer_compose.eval()
    surrogate_model_inner_compose.eval()
...
def get_grad_SHGP_dgl(flow, hg, adj_dict, surrogate_model, mask=None, verbose=True):
    ...
    for k, v in adj_dict.items():
        for kk, vv in v.items():
            grad_ = torch.autograd.grad(loss, vv, retain_graph=True, allow_unused=True)[0]
            source_ntype = k
            target_ntype = kk
    ...
    for etype in filtered_type:
        ...
        if verbose:
            print('grad fuse, the grad mean of [', etype, '] is', etype_grad.mean().item())

    return etype_grads_fused


def get_grad_HGCN_dgl(flow, hg, surrogate_model_inner_compose, surrogate_model_outer_compose, mask=None, verbose=True):
    surrogate_model_outer_compose.eval()
    surrogate_model_inner_compose.eval()
