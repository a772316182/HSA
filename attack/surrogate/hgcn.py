    ...
    def __init__(
        ...
        for i in range(len(meta_paths)):
            self.gat_layers.append(
                DenseGraphConv(in_size, hidden_size, activation=torch.nn.functional.relu, bias=False)
            )
        in_size = hidden_size * num_heads
        self.semantic_attention = SemanticAttention(in_size)
        self.predict = nn.Linear(in_size, out_size)

    def forward(self, A_list, X, return_reachable_adj=False, hg: dgl.DGLHeteroGraph = None, check=False):
        mx_list = []
        meta_path_Z_list = []
        for i in range(len(self.meta_paths)):
            ...
            # self loop
            mx = meta_path_reachable + torch.eye(meta_path_reachable.shape[0]).to(meta_path_reachable.device)
            mx[torch.where(mx > 1)] = 1
            if check:
                check_target = dgl.metapath_reachable_graph(hg.to(self.device),
                                                            metapath=meta_path).adj().to_dense() + torch.eye(
