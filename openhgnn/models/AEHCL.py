    ...
    def forward(self, event, pos_event, neg_event, neg_context, neg_entity, event_mask, type_num):
        ...
        for key in neg_entity.keys():
            if neg_entity_z is None:
                ...
                neg_entity_z = torch.cat((neg_entity_z, self.fc[key](neg_entity[key])),
                                         1)  # (batch,all_len,neg_num,out_dim)
        ...
        neg_matrix = torch.matmul(torch.unsqueeze(pos_entity_z, 2),
                                  torch.transpose(neg_entity_z, 2, 3))  # (batch,all_len,1,neg_num)
        ...
        lbl = torch.unsqueeze(torch.cat((torch.ones(pos_inter.shape[0]), torch.zeros(neg_inter.shape[0]))),
                              1)  # (2*batch,1)
