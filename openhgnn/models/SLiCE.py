...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask=None):
        ...
        q_s = self.W_Q(Q).view(batch_size, -1, self.n_heads, self.d_k).transpose(1,
                                                                                 2)  # 128(batcch)*4(head)*7(n_nodes)*64(d_k)
...
class SLiCE(BaseModel):
    ...
    def __init__(self,
        ...
        if not os.path.exists(args.pretrained_embeddings):
            ...
            for _ in range(10):
                ...
                walk = dgl.sampling.node2vec_random_walk(G, torch.tensor(nodes), 1, 1,
                                                         walk_length=80 - 1).tolist()  # len=walk_length+1
                walks.extend(walk)
    ...
    def forward(self, subgraph_list):
        ...
        for layer in self.layers:
            ...
            if self.fine_tuning_layer:
                try:
                    att_output = torch.cat((att_output, enc_self_attn.unsqueeze(0)),
                                           0)  # output attention of each layer
                except NameError:  # FIXME - replaced bare except
