...
class Evaluator():
    ...
    def mrr_(self, n_embedding, r_embedding, train_triplets, valid_triplets, test_triplets, score_predictor, hits=[],
             filtered='raw', eval_mode='test'):
        ...
        return cal_mrr(n_embedding, r_embedding, valid_triplets, test_triplets, self.triplets_to_filter,
                       score_predictor, hits, filtered, eval_mode)
...
def perturb_and_get_rank(n_embedding, r_embedding, eval_triplets, triplets_to_filter, score_predictor, filtered,
                         preturb_side):
...
def cal_mrr(n_embedding, r_embedding, valid_triplets, test_triplets, triplets_to_filter, score_predictor, hits=[],
            filtered='raw', eval_mode='test'):
    with th.no_grad():
        eval_triplets = test_triplets if eval_mode == 'test' else valid_triplets

        print('Perturbing subject...')
        ranks_s = perturb_and_get_rank(n_embedding, r_embedding, eval_triplets, triplets_to_filter, score_predictor,
                                       filtered, 's')
        print('Perturbing oubject...')
        ranks_o = perturb_and_get_rank(n_embedding, r_embedding, eval_triplets, triplets_to_filter, score_predictor,
                                       filtered, 'o')
        # get matrix
