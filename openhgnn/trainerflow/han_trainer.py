...
class HANNodeClassification(BaseFlow):
    ...
    def _full_train_step(self):
        ...
        h_dict = self.model.input_feature()
        logits = self.model(self.hg, h_dict)[self.category]
        loss = self.loss_fn(logits[self.train_idx], torch.squeeze(self.labels[self.train_idx]))
    ...
    def _full_test_step(self, modes, logits=None):
        ...
        with torch.no_grad():
            ...
            loss_dict = {key: self.loss_fn(logits[mask], torch.squeeze(self.labels[mask])).item() for key, mask in
                         masks.items()}
