...
def preprocess_amazon(self):
    ...
    in_dims = [hg.num_nodes('user'), hg.num_nodes('item'), hg.num_nodes('brand'), hg.num_nodes('category'),
               hg.num_nodes(
                   'view')]
    ...
    edge_dict = {'i': [ui_edge[0], ui_edge[1]], 'b': [ib_edge[0], ib_edge[1]], 'c': [ic_edge[0], ic_edge[1]],
...
def process(self):
    ...
    self.serach_adjs_pt = []
    if '0' in self.adjs_offset:
        self.serach_adjs_pt.append(sparse_mx_to_torch_sparse_tensor(
            normalize_sym(self.adjs_offset['0'] + sp.eye(self.adjs_offset['0'].shape[0], dtype=np.float32))))
    for i in range(1, int(max(self.adjs_offset.keys())) + 1):
        self.serach_adjs_pt.append(sparse_mx_to_torch_sparse_tensor(
            normalize_row(self.adjs_offset[str(i)] + sp.eye(self.adjs_offset[str(i)].shape[0], dtype=np.float32))))
    ...
    for k in range(num_node_types):
        i = torch.stack(
            (torch.arange(self.in_dims[k], dtype=torch.long), torch.arange(self.in_dims[k], dtype=torch.long)))
...
def train1(self):
    ...
    loss = - torch.mean(
        F.logsigmoid(torch.mul(out_s[self.train_pos[0].tolist()], out_t[self.train_pos[1].tolist()]).sum(dim=-1)) + \
...
def search(self):
    ...
    model_s = self.args.search_model(self.in_dims, self.args.attn_dim, len(self.serach_adjs_pt),
                                     [self.args.search_steps_s], self.cstr_source[self.args.dataset])
    model_t = self.args.search_model(self.in_dims, self.args.attn_dim, len(self.serach_adjs_pt),
                                     [self.args.search_steps_t], self.cstr_target[self.args.dataset])
...
def serach_train(self, model_s, model_t, optimizer_w,
    ...
    out_t = model_t(self.node_feats, self.node_types, self.serach_adjs_pt, idxes_seq_t, idxes_res_t)
    loss_a = - torch.mean(
        F.logsigmoid(torch.mul(out_s[self.val_pos[0].tolist()], out_t[self.val_pos[1].tolist()]).sum(dim=-1)) + \
