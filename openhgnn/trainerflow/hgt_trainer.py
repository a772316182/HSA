    ...
    def evaluate(self, split=None):
        ...
        with torch.no_grad():
            ...
            if mask is not None:
                ...
                losses = {key: self.loss_fn(avg_preds[mask], self.labels[mask].squeeze()) for key, mask in
                          masks.items()}
                print('Train:', metrics['train'], 'Validation:', metrics['val'], 'Test:', metrics['test'])
                print('Train:', metrics['train'], 'Validation:', metrics['val'], 'Test:', metrics['test'], file=log,
                      flush=True)
    ...
    def _test_step(self, split=None, logits=None):
        ...
        with torch.no_grad():
            ...
            if mask is not None:
                ...
                masks = {'train': self.train_idx, 'val': self.val_idx, 'test': self.test_idx}
                metrics = {key: self.evaluator(self.labels[mask].to('cpu'), logits[mask].argmax(dim=1).to('cpu')) for
